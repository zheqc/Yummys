(function() {
	...
	function signup() {
		...
		re = /^\w+$/;
		if (!re.test(signupusername.value)) {
		...
		if (signuppassword1.value != ""
			...
			if (signuppassword1.value.length < 5) {
				alert("Error: Password must contain at least five characters!");
				signuppassword1.focus();
				return false;
			}
			if (signuppassword1.value == signupusername.value) {
				alert("Error: Password must be different from Username!");
				signuppassword1.focus();
				return false;
			}
			re = /[0-9]/;
			if (!re.test(signuppassword1.value)) {
		...
		var params = 'user_id=' + username + '&password=' + password1 + '&first_name=' + firstname
					+ '&last_name=' + lastname;
		var req = JSON.stringify({});
	...
	function showSignUpSuccess() {
		$('signup-success').innerHTML = 'Successfully Registered!';
	}

	function clearSignUpSuccess() {
		$('signup-success').innerHTML = '';
	}

	function showSignUpError() {
		$('signup-error').innerHTML = 'Username Already Exists!';
	}
